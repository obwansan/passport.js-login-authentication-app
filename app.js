...
// When a a get request is sent to the "/secret" route, the isLoggedIn function is called before anything else happens (it's middleware). If the user's credentials are authenticated, the 'next' callback function is called, that renders the /secret page. If not, you're redirected back to the log in page and the 'next' function isn't called.
...
app.post("/register", function(req, res) {
  ...
  // The register method hashes the password (why it's not set as a property on the object passed to register() ) so the actual password isn't stored in the database.
...
// login logic
// Submitting the form generates a post request to this app.post("/login") route. The form data (i.e. username and password) are available as properties on the request object passed to the route callback (e.g. req.body.username).
// passport.authenticate() is middleware (i.e. code that runs between the request and response, or between the beginning and end of the route).
// passport.authenticate() tries to log the user in. It gets the username and password off request.body and authenticates them.
...
// The app.listen() function creates the Node.js web server at the specified host and port (in this case localhost:3000). It is identical to Node's http.Server.listen() method (when you run the command 'node app.js' this listens on port 3000 so the 'base' for all routes is localhost:3000)
